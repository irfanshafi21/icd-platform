  ...
  try {
    for (const width of process.env.QA_WIDTHS ? process.env.QA_WIDTHS.split(',').map(Number) : [1440, 768, 390]) {
      ...
      }
      {
        ...
        await page.locator('.notification-button').click(); await snap(page, width, 'notifications'); await page.locator('#mark-notifications-read').click(); assert.equal(await page.locator('.notification-summary b').innerText(), '0'); await page.locator('.notification-close').click(); report.checks.push({ width, name: 'notifications open, read, close', passed: true });
        ...
        await goRecruiter(page, 'screening'); await page.locator('#job-picker-trigger').click(); await snap(page, width, 'screening-picker'); await page.locator('[data-job-option="1"]').click(); assert.equal(await page.locator('#role').inputValue(), 'Senior Product Designer');
