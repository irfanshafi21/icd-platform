...
async function setup(browser, width, mode) {
  ...
  await context.route('**/*', async route => {
    ...
    if(api==='/api/insights'&&failInsight){failInsight=false;return route.fulfill({status:503,contentType:'application/json',body:JSON.stringify({detail:'Assistant temporarily unavailable. Please retry.'})});}
    ...
    if (api === '/api/organizations') result = [company, { id: 'qa-two', name: 'Arc Studio', industry: 'Design' }, { id: 'qa-three', name: 'Vertex Labs', industry: 'Technology' }, { id: 'qa-four', name: 'Meridian', industry: 'Consulting' }];
      ...
      {filename:'Invoice.pdf',reason:'This document could not be verified as a resume and was not saved. Upload a candidate resume/CV.'},
      {filename:'Duplicate.pdf',reason:'Already screened for this job. The original score is unchanged.'},
      {filename:'Retry.pdf',reason:'Provider temporarily unavailable. Please retry.'}
    ] };
...
async function snap(page, width, name) {
  ...
  if (width === 1440 && ['jobs', 'candidates', 'candidate-portal'].includes(name)) {
    ...
    const columns=name==='candidate-portal'?2:3,firstRow=row.slice(0,columns);
    check(width, `${name} shows ${columns} cards per desktop row`, firstRow.length === columns && new Set(firstRow.map(r => Math.round(r.left))).size === columns && Math.max(...firstRow.map(r => r.top)) - Math.min(...firstRow.map(r => r.top)) < 8 && (columns!==2||row[2].top>row[0].top), { row });
  }
  ...
  try {
    for (const width of process.env.QA_WIDTHS ? process.env.QA_WIDTHS.split(',').map(Number) : [1440, 768, 390]) {
      ...
      }
      {
        ...
        assert.equal(await page.locator('.shortlist-card .icd-score-ring').count(),3);
        assert.equal(await page.locator('.shortlist-metrics').count(),3);
        const ring=page.locator('.shortlist-card .icd-score-ring').first();
        assert.equal(await ring.innerText(),'92');
        assert.match(await ring.evaluate(el=>getComputedStyle(el).backgroundImage),/conic-gradient/);
        assert.equal(await ring.getAttribute('aria-label'),'ATS score: 92 out of 100');
        ...
        await page.locator('#files').setInputFiles({ name: 'QA-Resume.pdf', mimeType: 'application/pdf', buffer: Buffer.from('%PDF-1.4 mock resume for isolated UI test') }); await page.locator('.screen-submit').click(); await page.locator('#toast').filter({ hasText: '3 not processed' }).waitFor(); assert.equal(await page.locator('.page-screening').count(), 1); report.checks.push({ width, name: 'screening upload preserves screening page', passed: true }); await page.locator('#screening-outcome').waitFor(); assert.deepEqual(await page.locator('.outcome-metrics strong').allTextContents(),['1','1','1','1']); assert.equal(await page.locator('#screening-outcome li').count(),3); await snap(page,width,'screening-summary'); check(width,'screening outcome counts and reasons',true);
        ...
        await page.locator('#talent-search').fill('Sophia');
        await page.locator('.floating-ai').click();
        assert.equal(await page.locator('.floating-ai').evaluate(el=>getComputedStyle(el).borderRadius),'14px');
        assert.equal(await page.locator('.floating-ai').getAttribute('aria-expanded'),'true');
        controls.failNextInsight();
        await page.locator('#floating-ai-panel textarea').fill('Help me review the candidate evidence');
        await page.locator('[data-ai-send]').click();
        await page.locator('.floating-ai-error').filter({hasText:'temporarily unavailable'}).waitFor();
        assert.equal(await page.locator('#floating-ai-panel textarea').inputValue(),'Help me review the candidate evidence');
        await page.locator('[data-ai-send]:not([disabled])').waitFor();
        await page.locator('[data-ai-send]').click();
        await page.locator('[data-ai-send]:not([disabled])').waitFor();
        assert.equal(await page.locator('.floating-ai-error').isVisible(),false);
        await page.locator('#floating-ai-panel textarea').press('Escape');
        assert.equal(await page.locator('#floating-ai-panel').isVisible(),false);
        check(width,'Candidates AI opens after filtering, restores original shape, retries errors and closes with Escape',true);
        await goRecruiter(page, 'offers');
        await page.locator('#offer-all').uncheck();
        for (const id of ['prepare-offers','send-offers','preview-offer']) assert.equal(await page.locator('#'+id).isDisabled(), true);
        await page.locator('#offer-all').check();
        for (const id of ['prepare-offers','send-offers','preview-offer']) assert.equal(await page.locator('#'+id).isDisabled(), false);
        check(width, 'offer actions follow recipient selection', true);
        await page.locator('#offer-salary').fill('1200000 per annum'); await page.locator('#offer-start').fill('2026-10-01'); await page.locator('#offer-manager').fill('Taylor Rivers');
