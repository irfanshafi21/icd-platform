...
const candidate = await setup(browser, width, 'candidate'); await candidate.page.goto(origin + '/?candidate=1'); await candidate.page.locator('.candidate-portal').waitFor(); await snap(candidate.page, width, 'candidate-portal'); await candidate.page.locator('#search').fill('TypeScript'); assert.equal(await candidate.page.locator('[data-job]').count(), 1); await candidate.page.locator('#search').fill(''); check(width, 'candidate portal search filters jobs by skill', true); await candidate.page.locator('[data-job="1"]').click(); await candidate.page.locator('#apply').waitFor(); await snap(candidate.page, width, 'candidate-application'); await candidate.page.locator('[name="resume"]').setInputFiles({ name: 'QA-Resume.pdf', mimeType: 'application/pdf', buffer: Buffer.from('%PDF-1.4 mock resume for isolated UI test') }); await candidate.page.locator('#apply button').click(); await candidate.page.locator('.candidate-portal').waitFor(); assert.equal(await candidate.page.locator('[data-job="1"]').isDisabled(), true); await candidate.page.locator('[data-tab="apps"]').click(); await candidate.page.locator('.candidate-application-grid').filter({ hasText: 'Submitted' }).waitFor(); report.checks.push({ width, name: 'candidate apply records application and prevents duplicate apply', passed: true }); await candidate.context.close();
